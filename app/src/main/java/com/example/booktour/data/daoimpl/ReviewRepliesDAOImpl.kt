...
class ReviewRepliesDAOImpl(private val db: FirebaseFirestore) : ReviewRepliesDAO {
    ...
    override suspend fun docReplyTheoID(tourId: String, reviewId: String, idReply: String): Reply? {
        return try {
            val replySnapshot = collection
                .document(tourId)
                .collection("reviews")
                .document(reviewId)
                .collection("replies")
            ...
            if (replySnapshot.exists()) {
                replySnapshot.toObject(Reply::class.java)
            } else {
                null
            }
        } catch (e: Exception) {
            Log.e("test", "Lỗi khi đọc reply: $e")
            null
        }
    }
